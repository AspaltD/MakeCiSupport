...
class Cn_Tab0_FilePathSelect(itf.Itf_TabContainer):
    ...
    def _set_btmBtn_prop(self) -> itf.Data_BtmBtnPropsDict:
        ...
        props[en.BtmBtnIdx.NEXT_TAB] = props[en.BtmBtnIdx.NEXT_TAB].change_props(
            disabled=False,
            text="ReadCIF",
        )
        props[en.BtmBtnIdx.OTHER_FUNC1] = props[en.BtmBtnIdx.OTHER_FUNC1].change_props(
            disabled=False,
            text="ReadTXT"
        )
        props[en.BtmBtnIdx.OTHER_FUNC2] = props[en.BtmBtnIdx.OTHER_FUNC2].change_props(
            visible=False
        )
        return props
        ...
        for tab in self.cn_tabContents.controls:
            ...
            match tab.tabIdx.name:
                case 'FILE_PATH_SELECT':
                    props[en.BtmBtnIdx.NEXT_TAB] = props[en.BtmBtnIdx.NEXT_TAB].change_prop_click(self.event_tab0_next_readCIF)
                    props[en.BtmBtnIdx.OTHER_FUNC1] = props[en.BtmBtnIdx.OTHER_FUNC1].change_prop_click(self.event_tab0_func1_readTXT)
                case 'READ_DATA':
                    props[en.BtmBtnIdx.NEXT_TAB] = props[en.BtmBtnIdx.NEXT_TAB].change_prop_click(self.event_tab1_next_save_go)
                case 'BUILDER_LOG':
                    props[en.BtmBtnIdx.NEXT_TAB] = props[en.BtmBtnIdx.NEXT_TAB].change_prop_click(self.event_tab2_next_stop)
                case 'MI_PREVIEW':
                    props[en.BtmBtnIdx.OTHER_FUNC1] = props[en.BtmBtnIdx.OTHER_FUNC1].change_prop_click(self.event_tab5_func1_saveGJF)
                case _:
                    pass
        self.update()

    def left_btn_event(self, e:ft.ControlEvent):
        if not isinstance(e.control, itf.Itf_Left_TabBtn): raise TypeError("タブ変更用のボタンではありません")
...
def main(page: ft.Page):
    ...
    for name in en.FilePickerIdx:
        ...
        page.overlay.append(filePickers[name])
        page.update()
    print(filePickers)
    ...
    appLogger.info("app_init is finished.")
    page.update()
